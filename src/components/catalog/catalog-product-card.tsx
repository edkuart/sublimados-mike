...
const TONE_TO_KIND: Record<string, ProductKind> = {
  purple: "mug",
  teal:   "shirt",
  coral:  "pillow",
  ink:    "thermos",
...
export function CatalogProductCard({ product }: { product: CatalogProduct }) {
  ...
  const kind: ProductKind = (product.productKind as ProductKind) ?? TONE_TO_KIND[toneName] ?? "mug";
  const badgeClass = product.badge ? (BADGE_CLASSES[product.badge] ?? "bg-[var(--muted)] text-[var(--muted-foreground)]") : null;
    ...
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-white shadow-card transition-all hover:-translate-y-1 hover:shadow-card-hover">
        ...
        >
          ...
          <div className="absolute left-3 top-3 flex flex-col gap-1.5">
            ...
            {product.badge && badgeClass && (
              <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10.5px] font-bold ${badgeClass}`}>
                {product.badge}
              </span>
            )}
          </div>
      ...
      <div className="flex flex-1 flex-col p-4">
        <a href={`/productos/${product.slug}`}>
          <h3 className="text-[14.5px] font-extrabold leading-snug tracking-tight hover:text-[var(--primary)]">
            {product.name}
          </h3>
        </a>
        <p className="mt-1 line-clamp-2 text-[12px] leading-snug text-[var(--muted-foreground)]">
          {product.description}
        </p>
        ...
        <div className="mt-3 flex flex-wrap gap-1">
          {product.personalization.slice(0, 3).map((p) => (
            <span
              key={p}
              className="rounded-md bg-[var(--muted)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--muted-foreground)]"
            >
              {p}
            </span>
          ))}
        ...
        <div className="mt-3 flex items-end justify-between gap-2">
          ...
          <span className="inline-flex items-center gap-1 rounded-md bg-[var(--accent-muted)] px-2 py-1 text-[10px] font-bold text-[var(--accent-strong)]">
        ...
        >
          <MessageCircle size={14} aria-hidden="true" /> Cotizar
        </a>
      </div>
    </article>
  );
